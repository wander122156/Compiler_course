...
- [ ] Разбор if без else: ` if (true) {} ` → true
- [ ] Проверка выполнения тела if при TRUE условии: ` if (true) { 1 + 5 - 3 }  ` → 3
- [ ] Проверка выполнения тела if при FALSE условии: ` if (false) { 1 + 5 - 3 } ` → false
